...
def blrObjFunction(initialWeights, *args):
    ...
    w = initialWeights.reshape((initialWeights.shape[0], 1))
...
def blrPredict(W, data):
    ...
    label = np.argmax(sigmoid(np.dot(data, W)), axis=1).reshape(data.shape[0], 1)
...
# YOUR CODE HERE #
##################
train_with_labels = shuffle(np.hstack([train_data, train_label]))
train_sample = train_with_labels[:10000, :-1]
train_sample_label = train_with_labels[:10000, -1]
# validation_sample = train_with_labels[10000:20000, :-1]
# valid_label = train_with_labels[10000:20000, -1]
...
for c in range(10, 101, 10):
    ...
    print('\n RBF with c val: ' + str(c) + ' Validation set Accuracy:' + acc + '%')

    experiment_result_list.append(ExperimentResult(c, acc))

save_results(experiment_result_list)

# Testing with the full training dataset
...
print("Full data set training", acc)
